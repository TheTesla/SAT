...
Sat_Instance_t Sat_Apply_WorkingLiteral(Sat_Instance_t Sat_Instance)
{
    ...
    if(Sat_Instance.noClauses==(Sat_Instance.noEliminatedClauses)){
        ...
        cout << Sat_Instance.Literal.size()-1 << endl;
        for(x=1;x<Sat_Instance.Literal.size();x++){
            if(Sat_Instance.Literal[x].neg.set) cout << "-" << x << " ";
            if(Sat_Instance.Literal[x].pos.set) cout << x << " ";
        }
        cout << endl;
    }
    return Sat_Instance;
...
unsigned Sat_min_Lit(Sat_Instance_t Sat_Instance)
{
    ...
    // sehr langsam:
    /*
    for(x=1;x<Sat_Instance.noLiterals;x++){
        // Positive Literale prüfen reicht, da sonst Singe Polarity Literal -> sowieso eliminiert
        if(0<Sat_Instance.Literal[x].pos.noClauses) return x;
    }
    */
}
...
int main()
{
    ...
    SAT = readCNF("C:\\Projekte\\C++\\SAT\\easy\\aim-50-2_0-no-1.cnf");


    SAT = Sat_Update_Database(SAT);
    print_SAT_Instance_Infos(SAT);

    cout << "SAT? " << DPLLrec(SAT) << endl;
